...
import com.example.vishistvarugeese.memory.contacts.ContactContract.ContactEntry
...
class SQLiteHandler(context: Context?) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
     ...
     */
    fun addContacts(contact_details: ContactDetails) {
        val db = writableDatabase
        val values = ContentValues()
        values.put(ContactEntry.COLUMN_NAME, contact_details.getContactName()) // Name
        values.put(ContactEntry.COLUMN_PHONE_NUMBER, contact_details.getContactPhoneNumber()) // Phone Number
        values.put(ContactEntry.COLUMN_EMAIL, contact_details.getContactEmail()) // Email
        values.put(ContactEntry.COLUMN_PHOTO, contact_details.getImageAsString()) //photo
